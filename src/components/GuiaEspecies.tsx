...
const obtenerUrlImagenEspecie = (imagen?: string) => {
  ...
  return `http://localhost:3000/${imagen.startsWith("/") ? imagen.slice(1) : imagen}`
}
...
const GuiaEspecies: React.FC = () => {
  ...
  const filteredEspecies = especies.filter(especie => {
    ...
    const matchesSearch = searchQuery === '' || 
      (especie.nombresComunes && especie.nombresComunes.some(nombreObj => 
            ...
            >
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {especie.nombresComunes?.slice(0, 3).map((nombreObj, index) => (
                    <span
                      key={index}
