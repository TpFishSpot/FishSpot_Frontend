  ...
  if (especies.length === 0) {
    return (
      <div className="text-center py-12">
        <Fish className="w-16 h-16 mx-auto text-gray-400 dark:text-gray-500 mb-4" />
        <p className="text-gray-600 dark:text-gray-300 text-lg">No se registraron especies en este spot.</p>
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-2">¡Sé el primero en reportar una especie aquí!</p>
      </div>
    )
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {especies.map((especie) => (
        <Link key={especie.id} to={`/especie/${especie.id}`}>
          <div className="bg-white/20 dark:bg-gray-800/20 backdrop-blur-sm border border-white/30 dark:border-gray-700/30 rounded-xl p-4 
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-semibold text-gray-800 dark:text-gray-100 text-lg italic mb-1">
                  {especie.nombre_cientifico}
                </h4>

                {especie.descripcion && (
                  <p className="text-gray-600 dark:text-gray-300 text-sm mb-3 line-clamp-2">{especie.descripcion}</p>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {Array.isArray(especie.nombre_comun) && especie.nombre_comun.length > 0 ? (
                      ...
                      <span className="text-gray-500 dark:text-gray-400 text-xs italic">No especificados</span>
                    )}
                  </div>
                </div>
              </div>
